# Este script genera los metadatos y abstracts separados por categoría a partir del archivo de metadatos etiquetado [Consol_depur_tfg_etiquetado.csv] y de los archivos de abstracts totales almacenados en [Archivos_tfg_todas_univsdds]
# Con base en la etiqueta [categoría] asignada previamente a cada TFG en el archivo de metadatos, se realiza la separación y agrupación del total de tfg (*3 universidades) en las categorías previamente estab lecidas quedando alamcenadas en carpeta separada para cada categoría
# Genera el archivo [resumen_titulaciones_por_categoria.txt] que contiene la agrupación realizada de cada categoría con sus respectivas titulaciones
